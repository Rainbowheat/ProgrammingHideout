...
#define ERROR(cond, msg) assert((printf((cond) ? msg : '\0'), cond))

void errHnd(int value, char *message, int errorVal) {
  if (value == errorVal) {
    fprintf(stderr, "%s\n", message);
  }
}
...
int main(void) {
  ...
  memset(&hints, 0, sizeof(hints) / sizeof(char));
  ...
  error = getaddrinfo(0, "8080", &hints, &bind_address);
  ERROR(error == -1, "getaddrinfo failed");
  ...
  int socket_client =
      accept(socket_listen, (struct sockaddr *)&client_addresses, &client_len);
  ...
  getnameinfo((struct sockaddr *)&client_addresses, client_len, address_buffer,
